    ...
    <div class="course-page">
        ...
        <div v-else>
            ...
            <div v-if="!chooseHomework" class="materials-view">
                <div class="actions">
                    <button class="create-lesson" @click="openAddLessonModal">Создать урок</button>
                    <AddLessonModal ref="addLessonModal" :courseId="courseId" @lesson-created="onLessonCreated" />
                    <button class="upload-material" @click="openAddMaterialModal">Загрузить материал</button>
                    ...
                    <div v-for="(materials, lessonName) in course" :key="lessonName" class="lesson-card">
                        <div class="lesson-header">
                            ...
                            <div v-if="editingLesson === lessonName" class="edit-mode">
                                <input v-model="editedLessonName" type="text" placeholder="Название урока"
                                    class="lesson-name-input" />
                                <button @click="saveLesson(lessonName)" class="save-button">Сохранить</button>
                            </div>
                            ...
                            <div class="lesson-actions">
                                <button v-if="editingLesson !== lessonName" @click="editLesson(lessonName)"
                                    class="edit-button">
                                    Редактировать
                                </button>
                                <button v-if="editingLesson === lessonName" @click="removeLesson(lessonName)"
                                    class="delete-button">
                                    Удалить
                                </button>
                            </div>
                        ...
                        <ul class="material-list">
                            <li v-for="material in materials" :key="material.id" class="material-item">
                                <div v-if="editingLesson === lessonName" class="edit-material">
                                    <input v-model="editedMaterials[material.id]" type="text"
                                        class="material-name-input" />
                                </div>
                                <span v-else>{{ material.name }}</span>
                                <button v-if="editingLesson === lessonName" @click="deleteMaterial(material.id)"
                                    class="delete-button">
                                    Удалить
                                </button>
                            </li>
                ...
                <div v-else class="homework-list">
                    <div v-for="homework in homeworks" :key="homework.id" class="homework-item">
                        ...
                        <div class="buttons">
                            ...
                            <button @click="markAsChecked(homework.id)">
                                проверено
                            </button>
                        </div>
                    </div>
...
const getCourseDetails = async () => {
    try {
        const response = await axios.post(`${apiBase}/api/getCourseForTeacher`, { courseId })
...
const markAsChecked = async (homeworkId: number) => {
    const response = await axios.post(`${apiBase}/api/homework/checked`, { userProgressId: homeworkId })
...
const deleteMaterial = async (materialId: number) => {
    try {
        ...
        for (let lesson in course.value) {
            course.value[lesson] = course.value[lesson].filter((m: any) => m.id !== materialId)
        }
    } catch (error) {
